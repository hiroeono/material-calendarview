...
@Experimental
public class BiWeeklyPagerAdapter extends CalendarPagerAdapter<BiWeeklyView> {

    public BiWeeklyPagerAdapter(MaterialCalendarView mcv) {
        super(mcv);
    }

    @Override
    protected BiWeeklyView createView(int position) {
        return new BiWeeklyView(mcv, getItem(position), mcv.getFirstDayOfWeek());
    }

    @Override
    protected int indexOf(BiWeeklyView view) {
